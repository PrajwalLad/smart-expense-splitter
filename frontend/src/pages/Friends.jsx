        ...
        <div className="flex flex-col gap-3 w-full md:w-3/4">
          {friendArr.length === 0 ? (
            ...
            friendArr.map((friend) => (
              ...
              >
                <div className="md:flex items-center w-1/2 justify-between md:pr-10">
                  <h5 className="font-semibold text-lg">{friend.name}</h5>
                  {friend.youOwe > friend.owedToYou ? (
                    <span className="flex text-rose-700 font-semibold">-₹{friend.youOwe - friend.owedToYou}</span>
                  ) : (
                    <span className="flex text-green-900 font-semibold">+₹{friend.owedToYou - friend.youOwe}</span>
                  ) }
                </div>
